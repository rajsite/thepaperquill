...
$.fn.multimedia_portfolio = function(options) {
	...
	this.each(function(){ 	
		...
		for ( var i = 0; i < elements.length; i++ ) {
				...
				if ((currenthref= $(elements[i]).children().filter("a").attr('href'))!= null)	
				{
					if (currenthref.toLowerCase().indexOf('.flv') > 0) {
						var currentwidth = $(elements[i]).find('img').attr('width'); if (currentwidth==null) currentwidth='600';
						var currentheight = $(elements[i]).find('img').attr('height'); if (currentheight==null) currentheight='450';
		...
		$(window).bind("load",function(){
			 $(".ui-slider-1").slider({steps: elements.length*3, minValue: 0, maxValue: 50, startValue: 50, slide: function(e,ui) {caroussel_portfolio_vue((elements.length * ui.slider.curValue /100), portfolio, elements, dim3, portfolio_height, settings, def_element_width, false, dimensions_elements);}});
		});
